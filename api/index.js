...
app.post('/login',async(req,res)=>{
    ...
    if(passOk){
        jwt.sign({username,id:userDoc._id},secrate,{},(err,token)=>{
            if(err) throw err;
        ...
        res.status(400).json('Wrong Candentials')
    }
})

app.get('/profile',(req,res)=>{
    const {token}=req.cookies;
    jwt.verify(token,secrate,{},(err,info)=>{
        if(err) throw err;
        res.json(info);
    });
});
...
app.post('/post', upload.single('file'), async(req,res,next)=>{
    ...
    const {token}=req.cookies;
    jwt.verify(token,secrate,{},async(err,info)=>{
        if(err) throw err;
